 ...
 *
 * This file has been modified from its original version by Mmp, Inc. The original
...
/* package */ class CheckConfigration {

    public static String LOG_MESSAGE_TAG = "MmpAPI.ConfigurationChecker";
    ...
    public static boolean checkBasicConfiguration(Context context) {
        ...
        if (packageManager == null || packageName == null) {
            MmpLog.w(LOG_MESSAGE_TAG, "Can't check configuration when using a Context with null packageManager or packageName");
            return false;
        }
        if (PackageManager.PERMISSION_GRANTED != packageManager.checkPermission("android.permission.INTERNET", packageName)) {
            MmpLog.w(LOG_MESSAGE_TAG, "Package does not have permission android.permission.INTERNET - Mmp will not work at all!");
            MmpLog.i(LOG_MESSAGE_TAG, "You can fix this by adding the following to your AndroidManifest.xml file:\n" +
    ...
    public static boolean checkTakeoverInAppActivityAvailable(Context context) {
        if (mTakeoverActivityAvailable == null) {
            ...
            if (intentActivities.size() == 0) {
                MmpLog.w(LOG_MESSAGE_TAG, TakeoverInAppActivity.class.getName() + " is not registered as an activity in your application, so takeover in-apps can't be shown.");
                MmpLog.i(LOG_MESSAGE_TAG, "Please add the child tag <activity android:name=\"com.mmp.android.takeoverinapp.TakeoverInAppActivity\" /> to your <application> tag.");
                mTakeoverActivityAvailable = false;
                return mTakeoverActivityAvailable;
            }
    ...
    public static boolean checkPushNotificationConfiguration(Context context) {
        ...
        if (PackageManager.PERMISSION_GRANTED != packageManager.checkPermission("android.permission.INTERNET", packageName)) {
            MmpLog.w(LOG_MESSAGE_TAG, "Package does not have permission android.permission.INTERNET");
            MmpLog.i(LOG_MESSAGE_TAG, "You can fix this by adding the following to your AndroidManifest.xml file:\n" +
                    "<uses-permission android:name=\"android.permission.INTERNET\" />");
            return false;
        }
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                boolean extendsMpFcmService = MmpFCMMessagingService.class.isAssignableFrom(fcmClass);
        ...
        if (registeredServices.size() > 1) {
            MmpLog.w(LOG_MESSAGE_TAG, "You can't have more than one service handling \"com.google.firebase.MESSAGING_EVENT\" intent filter. " +
        ...
        try {
            ...
            MmpLog.w(LOG_MESSAGE_TAG, "Google Play Services aren't included in your build- push notifications won't work on Lollipop/API 21 or greater");
            MmpLog.i(LOG_MESSAGE_TAG, "You can fix this by adding com.google.android.gms:play-services as a dependency of your gradle or maven project");
        }
    ...
    public static boolean checkInstallReferrerConfiguration(Future<SharedPreferences> referrerPrefs) {
        try {
            ...
            MmpLog.d(LOG_MESSAGE_TAG, "Missing com.android.installreferrer dependency. Google Play Store referrer information won't be available.");
        } catch (InterruptedException e) {
            MmpLog.w(LOG_MESSAGE_TAG, "Could not read referrer shared preferences.");
