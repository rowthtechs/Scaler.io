...
/* package */ class DecideChecker {
    private static final String LOGTAG = "MmpAPI.DChecker";

    private final MmpConfig mConfig;
    private final Context mContext;
    private final Map<String, SelectMessages> mChecks;
    ...
    public DecideChecker(final Context context, final MmpConfig config) {
        mContext = context;
        mConfig = config;
        mChecks = new HashMap<String, SelectMessages>();
    ...
    public void addDecideCheck(final SelectMessages check) {
    ...
    public void runDecideCheck(final String token, final RemoteService poster) throws RemoteService.ServiceUnavailableException {
        SelectMessages updates = mChecks.get(token);
        if (updates != null) {
            ...
            try {
                ...
                MmpLog.e(LOGTAG, e.getMessage(), e);
            }
        }
    }
    ...
    private void setImages(final Iterator<InAppNotification> notificationIterator) throws RemoteService.ServiceUnavailableException {
        while (notificationIterator.hasNext()) {
            ...
            if (null == image) {
                MmpLog.i(LOGTAG, "Could not retrieve image for notification " + notification.getId() +
    ...
    private Result runDecideCheck(final String token, final String distinctId, final RemoteService poster)
        ...
        MmpLog.v(LOGTAG, "Mmp decide server response was:\n" + responseString);
    ...
    private static List<InAppNotification> parseInAppNotifications(JSONObject response) {
        ...
        if (response.has(NOTIFICATIONS)) {
            try {
                ...
                MmpLog.e(LOGTAG, "Mmp endpoint returned non-array JSON for " + NOTIFICATIONS + ": " + response);
            }
        }
            ...
            for (int i = 0; i < notifications.length(); i++) {
                try {
                    ...
                    if (notificationJson.has("display_triggers")) {
                        if (triggeredCount >= MmpConfig.MAX_EVENT_TRIGGERED_NOTIFICATION_CACHE_COUNT) {
                            continue;
                        } else {
                            triggeredCount++;
                        }
                    } else {
                        if (normalCount >= MmpConfig.MAX_NOTIFICATION_CACHE_COUNT) {
                            continue;
                        } else  {
                            normalCount++;
                        }
                    ...
                    MmpLog.e(LOGTAG, "Received a strange response from notifications service: " + notifications.toString(), e);
                } catch (final BadDecideObjectException e) {
                    MmpLog.e(LOGTAG, "Received a strange response from notifications service: " + notifications.toString(), e);
                } catch (final OutOfMemoryError e) {
                    MmpLog.e(LOGTAG, "Not enough memory to show load notification from package: " + notifications.toString(), e);
                }
            }
        }
    ...
    /* package */ static Result parseDecideResponse(String responseString)
        ...
        try {
            ...
            final String message = "Mmp endpoint returned unparsable result:\n" + responseString;
        ...
        if (response.has(EVENT_BINDINGS)) {
            try {
                ...
                MmpLog.e(LOGTAG, "Mmp endpoint returned non-array JSON for event bindings: " + response);
            }
        }
        ...
        if (response.has(VARIANTS)) {
            try {
                ...
                MmpLog.e(LOGTAG, "Mmp endpoint returned non-array JSON for variants: " + response);
            }
        }
        ...
        if (response.has(AUTOMATIC_EVENTS)) {
            try {
                ...
                MmpLog.e(LOGTAG, "Mmp endpoint returned a non boolean value for automatic events: " + response);
            }
        }
        ...
        if (response.has(INTEGRATIONS)) {
            try {
                ...
                MmpLog.e(LOGTAG, "Mmp endpoint returned a non-array JSON for integrations: " + response);
            }
        }
    ...
    private String getDecideResponseFromServer(String unescapedToken, String unescapedDistinctId, RemoteService poster)
        ...
        try {
            ...
            throw new RuntimeException("Mmp library requires utf-8 string encoding to be available", e);
        }
        ...
        try {
            properties.putOpt("android_lib_version", MmpConfig.VERSION);
            ...
            MmpLog.e(LOGTAG, "Exception constructing properties JSON", e.getCause());
        }

        final String checkQuery = queryBuilder.toString();
        final String url = mConfig.getDecideEndpoint() + checkQuery;

        MmpLog.v(LOGTAG, "Querying decide server, url: " + url);
    ...
    private Bitmap getNotificationImage(InAppNotification notification, Context context)
        ...
        for (String url : urls) {
            try {
                ...
                MmpLog.v(LOGTAG, "Can't load image " + url + " for a notification", e);
            }
        }
    ...
    private static byte[] checkDecide(RemoteService poster, Context context, String url)
        ...
        try {
            ...
            MmpLog.v(LOGTAG, "Cannot get " + url + ", file not found.", e);
        } catch (final IOException e) {
            MmpLog.v(LOGTAG, "Cannot get " + url + ".", e);
        } catch (final OutOfMemoryError e) {
            MmpLog.e(LOGTAG, "Out of memory when getting to " + url + ".", e);
        }

        return response;
    }

    public SelectMessages getDecideMessages(String token) {
