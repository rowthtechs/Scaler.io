 ...
 *
 * <p>You can use Mmp to capture and store referrer information,
 ...
 * all calls to {@link com.mmp.android.mpmetrics.MmpAPI#track(String, org.json.JSONObject)}
...
/* package */ class InstallReferrerPlay implements InstallReferrerStateListener {
    ...
    private static String TAG = "MmpAPI.InstallReferrerPlay";
        ...
        switch (responseCode) {
            case InstallReferrerClient.InstallReferrerResponse.OK:
                try {
                    ...
                    MmpLog.d(TAG, "There was an error fetching your referrer details.", e);
                    shouldRetry = true;
                }
                break;
            case InstallReferrerClient.InstallReferrerResponse.SERVICE_UNAVAILABLE:
                shouldRetry = true;
                MmpLog.d(TAG, "Service is currently unavailable.");
                break;
            case InstallReferrerClient.InstallReferrerResponse.SERVICE_DISCONNECTED:
                shouldRetry = true;
                MmpLog.d(TAG, "Service was disconnected unexpectedly.");
                break;
            case InstallReferrerClient.InstallReferrerResponse.FEATURE_NOT_SUPPORTED:
                MmpLog.d(TAG, "API not available on the current Play Store app.");
                break;
            case InstallReferrerClient.InstallReferrerResponse.DEVELOPER_ERROR:
                MmpLog.d(TAG, "Unexpected error.");
                break;
            default:
                break;
        }
    ...
    public void onInstallReferrerServiceDisconnected() {
        MmpLog.d(TAG, "Install Referrer Service Disconnected.");
        retryConnection();
    }
    ...
    public void connect() {
        try {
            ...
            MmpLog.e(TAG, "Install referrer client could not start connection", e);
        }
    }

    private void retryConnection() {
        if (mRetryCount > MAX_INSTALL_REFERRER_RETRIES) {
            MmpLog.d(TAG, "Already retried " + MAX_INSTALL_REFERRER_RETRIES + " times. Disconnecting...");
            disconnect();
            return;
        }
    ...
    public void disconnect() {
        if (mReferrerClient != null && mReferrerClient.isReady()) {
            try {
                ...
                MmpLog.e(TAG, "Error closing referrer connection", e);
            }
        }
    }
    ...
    /* package */ void saveReferrerDetails(String referrer) {
        ...
        PersistentIdentity.writeReferrerPrefs(mContext, MmpConfig.REFERRER_PREFS_NAME, newPrefs);
        ...
        if (matcher.find()) {
            ...
            if (null != encoded) {
                try {
                    ...
                    MmpLog.e(TAG, "Could not decode a parameter into UTF-8");
                }
            }
        }
        return null;
