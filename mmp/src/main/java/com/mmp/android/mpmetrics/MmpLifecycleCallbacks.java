...
/* package */ class MmpLifecycleCallbacks implements Application.ActivityLifecycleCallbacks {
    ...
    public MmpLifecycleCallbacks(MmpAPI mpInstance, MmpConfig config) {
    ...
    public void onActivityStarted(Activity activity) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= MmpConfig.UI_FEATURES_MIN_API && mConfig.getAutoShowMmpUpdates()) {
    ...
    private void trackCampaignOpenedIfNeeded(Intent intent) {
        ...
        if (intent.hasExtra("mp_campaign_id") && intent.hasExtra("mp_message_id")) {
            MmpAPI.trackPushNotificationEventFromIntent(mMpInstance.getContext(), intent, "$app_open");
        }
    }

    private final MmpAPI mMpInstance;
