...
// Will be called from both customer threads and the Mmp worker thread.
/* package */ class SelectMessages {

    public interface OnNewResultsListener {
        void onNewResults();
    }

    public SelectMessages(Context context, String token, OnNewResultsListener listener, UpdatesFromMmp updatesFromMmp, HashSet<Integer> notificationIds) {
        mContext = context;
        mToken = token;
        mListener = listener;
        mUpdatesFromMmp = updatesFromMmp;
    ...
    public synchronized void reportResults(List<InAppNotification> newNotifications,
        ...
        mUpdatesFromMmp.setEventBindings(eventBindings);
        ...
        for (int i = 0; i < newVariantsLength; i++) {
            try {
                ...
                MmpLog.e(LOGTAG, "Could not convert variants[" + i + "] into a JSONObject while comparing the new variants", e);
            }
        }
        ...
        if (hasNewVariants && mVariants != null) {
            ...
            for (int i = 0; i < newVariantsLength; i++) {
                try {
                    ...
                    MmpLog.e(LOGTAG, "Could not convert variants[" + i + "] into a JSONObject while updating the map", e);
                }
            }
        }
        ...
        mUpdatesFromMmp.storeVariants(mVariants);

        if (mAutomaticEventsEnabled == null && !automaticEvents) {
            MmpDbAdapter.getInstance(mContext).cleanupAutomaticEvents(mToken);
        }
        ...
        if (integrations != null) {
            try {
                ...
                MmpLog.e(LOGTAG, "Got an integration id from " + integrations.toString() + " that wasn't an int", e);
            }
        }

        MmpLog.v(LOGTAG, "New Decide content has become available. " +
    ...
    public synchronized InAppNotification getNotification(boolean replace) {
        if (mUnseenNotifications.isEmpty()) {
            MmpLog.v(LOGTAG, "No unseen notifications exist, none will be returned.");
            return null;
        }
        ...
        if (replace) {
            ...
            MmpLog.v(LOGTAG, "Recording notification " + n + " as seen.");
        }
        return n;
    }
    ...
    public synchronized InAppNotification getNotification(ResultMessagesAnalatic.EventDescription eventDescription, boolean replace) {
        if (mUnseenEventTriggeredNotifications.isEmpty()) {
            MmpLog.v(LOGTAG, "No unseen triggered notifications exist, none will be returned.");
            return null;
        }
        for (int i = 0; i < mUnseenEventTriggeredNotifications.size(); i ++) {
            ...
            if (n.matchesEventDescription(eventDescription)) {
                if (!replace) {
                    ...
                    MmpLog.v(LOGTAG, "recording triggered notification " + n.getId() +
                            " as seen " + eventDescription.getEventName());
                }
                return n;
            } else {
                MmpLog.v(LOGTAG, "triggered notification " + n.getId() +
    ...
    private static final String LOGTAG = "MmpAPI.DecideUpdts";
}
