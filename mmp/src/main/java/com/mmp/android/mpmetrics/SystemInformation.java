...
/* package */ class SystemInformation {
    ...
    private SystemInformation(Context context) {
        ...
        try {
            ...
            MmpLog.w(LOGTAG, "System information constructed with a context that apparently doesn't exist.");
        }
        ...
        if (null != hasSystemFeatureMethod) {
            try {
                ...
                MmpLog.w(LOGTAG, "System version appeared to support PackageManager.hasSystemFeature, but we were unable to call it.");
    ...
    public String getPhoneRadioType(Context context) {
        ...
        if (PackageManager.PERMISSION_GRANTED != packageManager.checkPermission("android.permission.READ_PHONE_STATE", packageName)) {
            MmpLog.w(LOGTAG, "Permission READ_PHONE_STATE not granted. Property $radio will not be available.");
            return null;
        }
    ...
    private static final String LOGTAG = "MmpAPI.SysInfo";
}
