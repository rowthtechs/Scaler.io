...
/**
 * An HTTP utility class for internal use in the Mmp library. Not thread-safe.
 */
public class HttpService implements RemoteService {

    private static boolean sIsMmpBlocked;
    private static final int MIN_UNAVAILABLE_HTTP_RESPONSE_CODE = HttpURLConnection.HTTP_INTERNAL_ERROR;
    private static final int MAX_UNAVAILABLE_HTTP_RESPONSE_CODE = 599;

    @Override
    public void checkIsMmpBlocked() {
        Thread t = new Thread(new Runnable() {
            public void run() {
                try {
                    InetAddress apiMmpInet = InetAddress.getByName("api.mmp.com");
                    InetAddress decideMmpInet = InetAddress.getByName("decide.mmp.com");
                    sIsMmpBlocked = apiMmpInet.isLoopbackAddress() ||
                            apiMmpInet.isAnyLocalAddress() ||
                            decideMmpInet.isLoopbackAddress() ||
                            decideMmpInet.isAnyLocalAddress();
                    if (sIsMmpBlocked) {
                        MmpLog.v(LOGTAG, "AdBlocker is enabled. Won't be able to use Mmp services.");
                    }
                } catch (Exception e) {
                }
            }
    ...
    public boolean isOnline(Context context, OfflineMode offlineMode) {
        ...
        try {
            ...
            if (netInfo == null) {
                isOnline = true;
                MmpLog.v(LOGTAG, "A default network has not been set so we cannot be certain whether we are offline");
            } else {
                isOnline = netInfo.isConnectedOrConnecting();
                MmpLog.v(LOGTAG, "ConnectivityManager says we " + (isOnline ? "are" : "are not") + " online");
            }
        } catch (final SecurityException e) {
            isOnline = true;
            MmpLog.v(LOGTAG, "Don't have permission to check connectivity, will assume we are online");
        }
        return isOnline;
    }
    ...
    private boolean onOfflineMode(OfflineMode offlineMode) {
        ...
        try {
            ...
            onOfflineMode = false;
            MmpLog.v(LOGTAG, "Client State should not throw exception, will assume is not on offline mode", e);
        }
    ...
    public byte[] performRequest(String endpointUrl, Map<String, Object> params, SSLSocketFactory socketFactory) throws ServiceUnavailableException, IOException {
        MmpLog.v(LOGTAG, "Attempting request to " + endpointUrl);
        ...
        while (retries < 3 && !succeeded) {
            ...
            try {
                ...
                MmpLog.d(LOGTAG, "Failure to connect, likely caused by a known issue with Android lib. Retrying.");
                retries = retries + 1;
        ...
        if (retries >= 3) {
            MmpLog.v(LOGTAG, "Could not connect to Mmp service after three retries.");
        }
        return response;
    }
    ...
    private static final String LOGTAG = "MmpAPI.Message";
}
