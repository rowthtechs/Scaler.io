...
/**
 * Handles translating events detected by ViewVisitors into events sent to Mmp
 *
...
/* package */ class DynamicEventTracker implements ViewVisitor.OnEventListener {
    ...
    public DynamicEventTracker(MmpAPI mmp, Handler homeHandler) {
        mMmp = mmp;
    ...
    public void OnEvent(View v, String eventName, boolean debounce) {
        ...
        try {
            ...
            MmpLog.e(LOGTAG, "Can't format properties from view due to JSON issue", e);
        }
        ...
        if (debounce) {
            ...
            mMmp.track(eventName, properties);
        }
    }
            ...
            synchronized (mDebouncedEvents) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (now - val.timeSentMillis > DEBOUNCE_TIME_MILLIS) {
                        mMmp.track(val.eventName, val.properties);
                        iter.remove();
                    }
                }
    ...
    private static String LOGTAG = "MmpAPI.DynamicEventTracker";
}
